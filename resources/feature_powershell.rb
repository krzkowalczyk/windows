...
action_class do
  ...
  def installed?
    @installed ||= begin
      cmd = powershell_out("Import-Module ServerManager; @(Get-WindowsFeature #{to_array(new_resource.feature_name).join(',')} | ?{$_.Installed -ne $TRUE}).count")
  ...
  def available?
    @available ||= begin
      cmd = powershell_out("Import-Module ServerManager; @(Get-WindowsFeature #{to_array(new_resource.feature_name).join(',')}).count")
      cmd.stderr.empty? && cmd.stdout.chomp.to_i > 0
    end
  end
end

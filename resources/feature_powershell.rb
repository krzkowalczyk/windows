...
action_class do
  ...
  def installed?
    @installed ||= begin
      cmd = powershell_out("(Get-WindowsFeature #{to_array(new_resource.feature_name).join(',')} | ?{$_.Installed -ne $TRUE}).count")
  ...
  def available?
    @available ||= begin
      cmd = powershell_out("(Get-WindowsFeature #{to_array(new_resource.feature_name).join(',')} | ?{$_.Installed -ne $FALSE}).count")
      cmd.stderr.empty? && cmd.stdout.chomp.to_i > 0
    end
  end
end
